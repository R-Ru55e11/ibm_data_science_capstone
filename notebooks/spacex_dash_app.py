...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.RangeSlider(id='payload-slider',
                                                ...
                                                marks={x : str(x) for x in range(0, 11000, 2500)},
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'All':
        ...
        names='Launch Site',
        title='Percentage Launch Success for All Sites')
        return fig
    else:
        # return the outcomes piechart for a selected site
        fig = px.pie(filtered_df, values=filtered_df.groupby(['class']).size(), 
        names=['Fail', 'Pass'], 
        title='Launch Success and Failure Percentages for site '+ entered_site)
        return fig
...
def get_scatter_chart(site, payload):
    ...
    if site == 'All':
        spacex_dff = spacex_df[(spacex_df['Payload Mass (kg)']>=payload[0]) & (spacex_df['Payload Mass (kg)']<=payload[1])]
        fig = px.scatter(
            spacex_dff, 
            x='Payload Mass (kg)', 
            y='class', 
